...
class ProductListbeApp extends StatelessWidget {
  ...
  ProductListbeApp({required this.user}); // سازنده با User
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      //صفحه اصلی برنامه
      home: ProductListbeScreen(user: user), // ارسال User به صفحه لیست محصولات
    );
  }
}
...
class ProductListbeScreen extends StatefulWidget {
  final User user; // دریافت User به عنوان ورودی
  ProductListbeScreen({required this.user}); // سازنده با User
...
//متغیر سرچ که فعلا یک استرینگ خالی است ینی تا زمانی که کاربر چیزی وارد نکند، تمام محصولات نمایش داده می‌شن
  ...
  Widget build(BuildContext context) {
    ...
    List<Cake> filteredCakes = getCakesByCategory(CakeCategory.bento).where((cake) {
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            widget.product['isFavorite'] = isFavorite; // ذخیره وضعیت در محصول
                            if (isFavorite) {
