...
class ProductListfoApp extends StatelessWidget {
  ...
  ProductListfoApp({required this.user}); // سازنده با User
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      //صفحه اصلی برنامه
      home: ProductListfoScreen(user: user), // ارسال User به صفحه لیست محصولات
    );
  }
}
...
class ProductListfoScreen extends StatefulWidget {
  ...
  final User user; // دریافت User به عنوان ورودی
  ProductListfoScreen({required this.user}); // سازنده با User
...
//متغیر سرچ که فعلا یک استرینگ خالی است ینی تا زمانی که کاربر چیزی وارد نکند، تمام محصولات نمایش داده می‌شن
  ...
  Widget build(BuildContext context) {
    ...
    List<Cake> filteredCakes = getCakesByCategory(CakeCategory.fondant).where((cake) {
